...
- **行内查询模式 (Inline Mode)**：支持在任意聊天中输入 `@Bot用户名 <分享文案或链接>` 呼出选单，无需将 Bot 拉入群组即可一键发送无水印视频或图集。自动过滤短视频 App 复制出的杂乱口令、时间戳与标签。
...
若希望在任意群聊、频道或私聊中通过 `@Bot` 直接选单发送视频：

1. 打开 Telegram 并私聊 [@BotFather](https://t.me/BotFather)；
2. 发送 `/setinline` 指令；
3. 选择你的机器人；
4. 设置占位提示符（Placeholder），例如输入：`粘贴抖音或TikTok分享文本...`；
5. 在任意聊天框中输入 `@你的Bot用户名 <抖音/TikTok分享文本>`，等待 1 秒即可弹出解析结果并一键发送！
